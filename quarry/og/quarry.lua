...
excessFuelAmount = 30000 --How much fuel the turtle will get maximum. Limited by turtle.getFuelLimit in recent CC [Default math.huge]
...
maxTries = 2000 --How many times turtle will try to dig a block before it "counts" bedrock [Default 200]
...
--Mining functions
function dig(doAdd, mineFunc, inspectFunc, suckDir,blacklistCheck)
  --Note, turtle will not bother comparing if not given an inspectFunc
  os.sleep(0.1)
  if doAdd == nil then
    doAdd = true
  end
  mineFunc = mineFunc or turtle.dig
  local function retTab(tab)
    if type(tab) == "table" then
      return tab
    end
  end --Please ignore the stupid one-line trickery. I felt special writing that. (Unless it breaks, then its cool)
  ...
  if oreQuarry and inspectFunc and blacklistCheck then
  ...
  if not oreQuarry or not inspectFunc or mineFlag then
    ...
    while success and continueDigging  and attempts < 50 do
      attempts = attempts + 1
      continueDigging = not mineFunc()
      os.sleep(0.2)
      if inspectFunc then
        success, data = inspectFunc()
      else
...
function digUp(doAdd, ignoreInspect)--Regular functions :) I switch definitions for optimization (I think)
  return dig(doAdd, turtle.digUp, turtle.inspectUp, "up", true)
end
function digDown(doAdd, ignoreInspect)
  return dig(doAdd, turtle.digDown, turtle.inspectDown, "down", true)
end
...
function smartDig(doDigUp, doDigDown) --This function is used only in mine when oldOreQuarry
  ...
  if blockAbove then dig(true, turtle.digUp, turtle.inspectUp, "up") end
  if blockBelow then dig(true, turtle.digDown, turtle.inspectDown, "down") end
end
...
function mine(doDigDown, doDigUp, outOfPath,doCheckInv) -- Basic Move Forward
  ...
  local count = 0
  if not outOfPath then dig(false, turtle.dig, turtle.inspect) end  --This speeds up the quarry by a decent amount if there are more mineable blocks than air
  while not forward(not outOfPath) do
    sleep(0) --Calls coroutine.yield to prevent errors
    count = count + 1

    if not dig(false, turtle.dig, turtle.inspect) then
      attack()
    end
    if count > 10 then
      attack()
...
function midRunRefuel(i, allowed)
  ...
  turtle.refuel(64) --The refueling part of the the doRefuel option
    ...
    repeat
      if not forward(false) then --Tries to go back out the way it came
        if not attck() then --Just making sure not mob-blocked
          if not dig(false, turtle.dig, turtle.inspect) then --Now we know its bedrock
